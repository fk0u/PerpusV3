...
if (isset($_GET['delete'])) {
    ...
    // Pastikan hanya akun siswa yang bisa dihapus
    try {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $error_message = 'Tidak dapat menghapus pengguna ini.';
        }
    } catch (PDOException $e) {
...
// Ambil Data Pengguna (Hanya Siswa)
...
<head>
  ...
  <style>
    ...
    /* Form Styles */
    .form-input {
      @apply block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-primary focus:ring-primary dark:bg-gray-700 dark:text-white sm:text-sm;
    }
    ...
    <main class="flex-grow py-6">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
          <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white" data-aos="fade-up">Kelola Pengguna Siswa</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400" data-aos="fade-up" data-aos-delay="100">
              Kelola akun siswa perpustakaan
            </p>
          </div>
          <div class="mt-4 md:mt-0" data-aos="fade-up" data-aos-delay="200">
            <button id="add-user-btn" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
          ...
          <div class="flex flex-col sm:flex-row gap-4 items-center justify-between">
            <div class="w-full sm:w-auto">
              ...
              <select id="class-filter" class="form-select">
                ...
                <?php
                // Get unique classes
                $classes = array_unique(array_column($users, 'class'));
            ...
            <div class="w-full sm:w-64">
              ...
              <div class="relative rounded-md shadow-sm">
                <input type="text" id="search" class="form-input pl-10" placeholder="Cari nama atau username...">
                  ...
                  <tr>
                    <td colspan="6" class="px-6 py-12 text-center">
                      <div class="flex flex-col items-center">
                        ...
                        <p class="text-gray-500 dark:text-gray-400 text-lg font-medium">Belum ada data siswa</p>
                        <p class="text-gray-400 dark:text-gray-500 text-sm mt-1">Tambahkan siswa baru dengan klik tombol "Tambah Pengguna"</p>
                      </div>
                    </td>
                  </tr>
                <?php else: ?>
                  <?php foreach ($users as $user): ?>
                    <tr class="user-row" data-class="<?= htmlspecialchars($user['class']); ?>">
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white"><?= htmlspecialchars($user['id']); ?></td>
                      ...
                      <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <button class="text-primary hover:text-primary/80 mr-3 view-user" data-id="<?= $user['id']; ?>">
                          <i class="lni lni-eye"></i>
                        </button>
                        <button class="text-danger hover:text-danger/80 delete-user" data-id="<?= $user['id']; ?>" data-name="<?= htmlspecialchars($user['full_name']); ?>">
            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p class="text-sm text-gray-700 dark:text-gray-300">
                  Menampilkan <span class="font-medium"><?= count($users) ?></span> dari <span class="font-medium"><?= count($users) ?></span> data
                </p>
              </div>
              <div>
      ...
      <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
        <form method="POST" action="" id="add-user-form">
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                ...
                <div class="mt-4 space-y-4">
                  <div class="form-floating">
                    <input type="text" class="form-input" id="full_name" name="full_name" placeholder=" " required>
                  ...
                  <div class="form-floating">
                    <input type="text" class="form-input" id="class" name="class" placeholder=" " required>
          ...
          <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            ...
            <button type="button" id="cancel-add-user" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 dark:border-gray-600 shadow-sm px-4 py-2 bg-white dark:bg-gray-800 text-base font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
              Batal
            </button>
          </div>
        </form>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      addUserBtn.addEventListener('click', function() {
        ...
        gsap.from(addUserModal.querySelector('div > div:last-child'), {
          duration: 0.5,
          y: 50,
          opacity: 0,
          ease: 'power3.out'
      ...
      cancelAddUser.addEventListener('click', function() {
        gsap.to(addUserModal.querySelector('div > div:last-child'), {
          duration: 0.3,
          y: 50,
          opacity: 0,
          ease: 'power3.in',
      ...
      viewButtons.forEach(button => {
        button.addEventListener('click', function() {
          ...
          Swal.fire({
            ...
            text: `Melihat detail pengguna dengan ID: ${userId}`,
            icon: 'info'
          });
        });
      });
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function filterTable() {
        ...
        userRows.forEach(row => {
          ...
          // Class filter
          let showByClass = classValue === 'all' || rowClass === classValue;
