...
if (isset($_GET['unique_code']) || (isset($_POST['unique_code']) && !empty($_POST['unique_code']))) {
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE rr.unique_code = ? AND rr.verified = 0
    ");
    ...
    if ($book_data) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("
                ...
                WHERE id = ?
            ");
            $stmt->execute([$condition, $notes, $borrow_id]);
            
            // Update book stock (increment by 1)
            $stmt = $pdo->prepare("UPDATE books SET stock = stock + 1 WHERE id = ?");
            $stmt->execute([$book_data['book_id']]);
            
            // Mark return request as verified
            $stmt = $pdo->prepare("UPDATE return_requests SET verified = 1 WHERE borrow_history_id = ?");
            $stmt->execute([$borrow_id]);
            ...
            $pdo->rollBack();
            $error_message = 'Terjadi kesalahan saat memproses pengembalian buku: ' . $e->getMessage();
        }
    } else {
